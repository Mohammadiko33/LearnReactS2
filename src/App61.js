...
export default function App58() {
    ...
    if (userName.length > 2 && password.length > 4 && userEmail.length > 12) {
      ...
      try {
        ...
        if (res.status === 200) {
          ...
          const userId = Object.keys(data)[0]; // دریافت شناسه کاربر از پاسخ
          const formattedUser = { id: userId, ...newUser };

          setUsers([...users, formattedUser]); // به‌روزرسانی کاربران با کاربر جدید
  ...
  return (
    <>
      ...
      <NextGetUsers users={users} setUsers={setUsers} />
    </>
  );
}
